...
from utils import SteinGen_run_multi,SteinGen_run,SteinGen_run_nr, gkss_, compute_gkss, compute_hamming, gen_cell, compute_e2st
...
with mp.Pool(n_cpus) as pool:
    res = pool.map(fn, [Xs[i]  for i in range(n)])

print(str(model_name), d, r_sample, "finish!")
np.savez("../res/"+str(model_name)+"n"+str(d)+"gen_samples_size.npz",res = res, Xs = Xs) #res is a n x 3 x 20 x b x d x d; 20 for k_length
...
#         np.savez("../res/"+str(model_name)+"n"+str(d)+"gen_sna_samples_size.npz", Xsna = Xsna, Xs = Xs, MC_coef=MC_coef) #res is a 3 x n x b x d x d; 20 for k_length
   

# np.savez("../res/"+str(model_name)+"n"+str(d)+"r"+str(r_sample)+"gen_samples_size_overall.npz", Xsna = Xsna, Xs = Xs, MC_coef=MC_coef, res=res) #res is a 3 x n x b x d x d; 20 for k_length
